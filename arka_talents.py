...
def unified_ask(
    ...
    *,
    ...
    try:
        from arka_turboquant_rag import search_documents, use_turboquant
    ...
    if youtube or _wants_youtube(question):
        try:
            from arka_youtube_research import cmd_research
            import io
    ...
    try:
        if deep:
            from arka_chat import scrape_search_results
            ...
            from arka_chat import snippet_lookup
...
def speak_research(query: str, *, limit: int = 5, speak: bool = True) -> int:
    ...
    py = _py()
    proc = subprocess.run(
        [py, str(FISH_DIR / "arka_youtube_research.py"), "search", query, "--limit", str(limit)],
        capture_output=True,
        text=True,
...
def handoff_run_with_notify(*, limit: int = 3) -> int:
    from arka_agent import handoff_run
...
def main() -> int:
    ...
    if args.cmd == "ask":
        ...
        from arka_predictions import run_prediction, list_history
