...
if test -f ~/.config/fish/.env
    for line in (cat ~/.config/fish/.env | grep -v '^#' | grep -v '^\s*$')
...
if status is-interactive
    ...
    function h
        ...
        echo "AGENT_NAME=arka in .env (default); AGENT_WAKE_AUTO=1 to start listener on shell open"
...
function _arka_speak_lang --description "Set or list Arka voice language (internal)"
    ...
    set -l code (python3 ~/.config/fish/indic_tts.py resolve-lang $argv[1] 2>/dev/null)
    if test -z "$code"
    ...
    if test -f ~/.config/fish/.env
        set -l env_content (cat ~/.config/fish/.env | grep -v '^ARKA_SPEAK_LANG=')
        printf "%s\n" $env_content > ~/.config/fish/.env
    end
    echo "ARKA_SPEAK_LANG=$code" >> ~/.config/fish/.env
...
function _arka_speak_voice --description "Set or list Arka neural voice (internal)"
    if test (count $argv) -eq 0
        python3 ~/.config/fish/edge_speak.py voices
        return 0
    end
    set -l voice $argv[1]
    set -gx ARKA_SPEAK_VOICE $voice
    if test -f ~/.config/fish/.env
        set -l env_content (cat ~/.config/fish/.env | grep -v '^ARKA_SPEAK_VOICE=')
        printf "%s\n" $env_content > ~/.config/fish/.env
    end
    echo "ARKA_SPEAK_VOICE=$voice" >> ~/.config/fish/.env
...
function _arka_usage_autostart_ensure --description "Install XDG autostart for usage tracker on login (internal)"
    ...
    set -l script ~/.config/fish/arka_usage.py
    mkdir -p $dir
    printf '%s\n' \
        '[Desktop Entry]' \
        'Type=Application' \
...
function _arka_start_all --description "Start remote server + wake listener (internal)"
    ...
    if test $quiet -eq 0
        ...
        bash ~/.config/fish/arka_boot.sh start
        sleep 1
        _arka_status_all
    else
        env ARKA_START_QUIET=1 bash ~/.config/fish/arka_boot.sh start 2>/dev/null
    end
end

function _arka_stop_all --description "Stop all Arka background services (internal)"
    bash ~/.config/fish/arka_boot.sh stop
    _agent_usage_stop
...
function _arka_autostart_install --description "Enable Arka on PC boot via systemd (internal)"
    ...
    set -l enable_listen 1
    if set -q ARKA_REMOTE_AUTO; and test "$ARKA_REMOTE_AUTO" = 0 -o "$ARKA_REMOTE_AUTO" = false
        set enable_remote 0
    end
    if set -q AGENT_WAKE_AUTO; and test "$AGENT_WAKE_AUTO" = 0 -o "$AGENT_WAKE_AUTO" = false
        set enable_listen 0
    end
...
" 2>/dev/null)
    ...
    echo "#   mkdir -p ~/arka && scp YOU@PC_IP:~/.config/fish/arka_phone.py ~/arka/"
    echo "#   cp ~/.config/fish/termux-boot-arka.sh ~/.termux/boot/arka.sh  # optional boot ping"
end

function _agent_remote_start --description "Start Arka remote server for phone STT/TTS (internal)"
    set -l script ~/.config/fish/arka_remote_server.py
...
function _agent_remote_stop --description "Stop Arka remote server (internal)"
    set -l pidfile ~/.cache/fish-agent/arka_remote.pid
    python3 ~/.config/fish/arka_remote_server.py stop 2>/dev/null
    if test -f "$pidfile"
        set -l pid (cat "$pidfile" 2>/dev/null)
        kill "$pid" 2>/dev/null
        rm -f "$pidfile"
...
function speak_aloud --description "Speak text aloud (Sarvam → neural edge → Parler → system)"
    ...
    if test $try_sarvam -eq 1; and test -n "$SARVAM_API_KEY"
        python3 ~/.config/fish/sarvam_speak.py "$text"
        if test $status -eq 0
            return 0
        end
    ...
    if test $try_edge -eq 1
        python3 ~/.config/fish/edge_speak.py speak "$text"
        if test $status -eq 0
            return 0
        end
    ...
    if test $try_parler -eq 1
        python3 ~/.config/fish/indic_tts.py speak "$text"
        if test $status -eq 0
            return 0
        end
...
function _arka_python --description "Python interpreter for Arka agents/LLM (internal)"
    set -l vpy ~/.config/fish/venv-arka/bin/python3
    if test -x $vpy
        echo $vpy
    else
        echo python3
...
function _agent_llm_complete --description "LLM system+user prompt via Agno; prints response text (internal)"
    ...
    set -l out ($py ~/.config/fish/arka_llm.py complete --system "$system_text" --user "$user_text" --temperature $temperature 2>/dev/null)
    if test -n "$out"
...
function _agent_llm_route --description "Interpret NL command to skill/shell via Agno (internal)"
    ...
    set -l out ($py ~/.config/fish/arka_llm.py route "$cmd" --skills "$available_skills" 2>/dev/null)
    if test -n "$out"
        _agent_clean_llm_output "$out"
        return 0
    end
...
function ai-status --description "Show current AI provider and model in use"
    ...
    if test -z "$primary_provider"
        ...
        echo ""
        echo (set_color yellow)"  To configure, add to ~/.config/fish/.env:"(set_color normal)
...
function ai-pref --description "Set preferred AI provider and model"
    ...
    if test "$arg1" = "clear"
        ...
        if test -f ~/.config/fish/.env
            set -l env_content (cat ~/.config/fish/.env | grep -v '^AI_PREFERRED_PROVIDER=' | grep -v '^AI_PREFERRED_MODEL=')
            printf "%s\n" $env_content > ~/.config/fish/.env
        end
        echo (set_color green)"✓ Preference cleared and removed from ~/.config/fish/.env permanently"(set_color normal)
        return 0
    end
    ...
    if test -n "$model"
        ...
        # Save permanently to ~/.config/fish/.env
        if test -f ~/.config/fish/.env
            set -l env_content (cat ~/.config/fish/.env | grep -v '^AI_PREFERRED_PROVIDER=' | grep -v '^AI_PREFERRED_MODEL=')
            printf "%s\n" $env_content > ~/.config/fish/.env
            echo "AI_PREFERRED_PROVIDER=$provider" >> ~/.config/fish/.env
            echo "AI_PREFERRED_MODEL=$model" >> ~/.config/fish/.env
        else
            echo "AI_PREFERRED_PROVIDER=$provider" > ~/.config/fish/.env
            echo "AI_PREFERRED_MODEL=$model" >> ~/.config/fish/.env
        end
...
function spotify_control --description "Control Spotify playback (play/pause/next/prev/status/dom)"
    ...
    switch $action
        ...
        case '*'
            ...
            echo "  spotify_control dom           # save ~/.config/fish/spotify-dom.html"
...
function weather --description "Get current weather or N-day forecast (Open-Meteo)"
    set -l py (_arka_python)
    if test (count $argv) -eq 0
        $py ~/.config/fish/arka_chat.py weather
        return $status
    end
    $py ~/.config/fish/arka_chat.py weather $argv
end
...
function app_usage --description "Show which apps you used and for how long"
    ...
    if test (count $argv) -ge 1
        switch $argv[1]
            ...
            case start
                python3 ~/.config/fish/arka_usage.py start
                return $status
            case stop
                python3 ~/.config/fish/arka_usage.py stop
                return $status
            case '*'
                set period $argv[1]
        end
    end
    set -l out (python3 ~/.config/fish/arka_usage.py report $period 2>&1)
    if test $status -ne 0
...
function _agent_usage_start --description "Start app + website usage tracker (internal)"
    ...
    python3 ~/.config/fish/arka_usage.py start 2>/dev/null
end

function _agent_usage_stop --description "Stop app usage tracker (internal)"
    python3 ~/.config/fish/arka_usage.py stop 2>/dev/null
end
...
function whatsapp_listen --description "Watch WhatsApp inbox → Arka (Selenium listen + pywhatkit send)"
    set -l script ~/.config/fish/arka_whatsapp_inbox.py
    set -l py (_arka_python)
    if not test -f "$script"
        echo (set_color red)"Missing $script"(set_color normal)
        return 1
...
function internet_enhance --description "Artificial Internet Enhancements (AIE): automate desktop/internet helpers"
    set -l py (_arka_python)
    if test (count $argv) -eq 0
        $py ~/.config/fish/arka_aie.py status
        return $status
    end
    switch $argv[1]
        case help -h --help
        ...
        case status start stop cleanup list
            $py ~/.config/fish/arka_aie.py $argv
            return $status
        case '*'
...
function youtube_bulk --description "Bulk download YouTube playlists/channels (yt-dlp + web UI)"
    set -l py (_arka_python)
    if test (count $argv) -eq 0
        $py ~/.config/fish/arka_youtube_bulk.py status
        return $status
    end
    switch $argv[1]
        case help -h --help
        ...
        case status start stop open download library logs
            $py ~/.config/fish/arka_youtube_bulk.py $argv
            return $status
        case '*'
            echo (set_color red)"Unknown subcommand: $argv[1]"(set_color normal)
            youtube_bulk help
...
function youtube_download --description "Download a single YouTube video or shorts link (yt-dlp)"
    ...
    set -l py (_arka_python)
    $py ~/.config/fish/arka_youtube.py download $argv
end
...
function _arka_summarize_media --description "Transcribe + summarize a local media file (internal)"
    ...
    $py ~/.config/fish/arka_media.py summarize "$file" -q "$q" --save $yt_args
end
...
function generate_image --description "Generate images via Google Nano Banana (Gemini API) or Pollinations"
    ...
    set -l py (_arka_python)
    $py ~/.config/fish/arka_generate_image.py $flags -- "$prompt"
end
...
function generate_video --description "Generate real AI video (Pollinations or Gemini Veo — no fake slideshows)"
    ...
    set -l prompt (string join ' ' -- $prompt_parts)
    set -l py (_arka_python)
    $py ~/.config/fish/arka_generate_video.py $flags -- "$prompt"
end
...
function youtube_transcript --description "Fetch or summarize a YouTube video transcript"
    ...
    set -l py (_arka_python)
    $py ~/.config/fish/arka_youtube.py $argv
end
...
function media_transcript --description "Transcribe or summarize local mp3/mp4/audio/video"
    ...
    if test "$argv[1]" = --setup-local
        set -l py (_arka_python)
        $py ~/.config/fish/arka_media.py setup-local
        return $status
    end
    set -l py (_arka_python)
    ...
    if test "$action" = summarize
        $py ~/.config/fish/arka_media.py summarize $args
    else
        $py ~/.config/fish/arka_media.py transcript $args
    end
end
...
function folder_summarize --description "Summarize all media files in a folder into one digest"
    ...
    set -l py (_arka_python)
    $py ~/.config/fish/arka_batch_summarize.py folder $argv
end
...
function playlist_summarize --description "Summarize a YouTube playlist URL or local playlist folder"
    ...
    set -l py (_arka_python)
    $py ~/.config/fish/arka_batch_summarize.py playlist $argv
end
...
function youtube_research --description "Search YouTube, summarize all result transcripts into one research brief"
    ...
    set -l py (_arka_python)
    if test "$argv[1]" = list
        $py ~/.config/fish/arka_youtube_research.py list
        return $status
    end
    if test "$argv[1]" = check verify test
        $py ~/.config/fish/arka_youtube_research.py check
        return $status
    end
    if test "$argv[1]" = ask
        ...
        $py ~/.config/fish/arka_youtube_research.py ask $argv[2] $argv[3..-1]
        return $status
    end
    $py ~/.config/fish/arka_youtube_research.py search $argv
end
...
function summarize_url --description "Summarize a web page or article URL"
    ...
    set -l py (_arka_python)
    $py ~/.config/fish/arka_summarize.py $argv
end
...
function disk_breakdown --description "Show disk space used by videos, pictures, documents, etc."
    ...
    set -l out (python3 ~/.config/fish/arka_disk.py breakdown "$target" 2>&1)
    if test $status -ne 0
        echo (set_color red)"$out"(set_color normal)
        return 1
    end
...
function pdf_list --description "List ingested documents (filename + artifact id)"
    set -l py (_arka_python)
    $py ~/.config/fish/arka_pdf_rag.py list
end
...
function pdf_ingest --description "Ingest a document; auto-starts PrivateGPT and Qdrant if needed"
    if test (count $argv) -eq 0
        ...
        echo "Formats: arka pdf formats  |  python3 ~/.config/fish/arka_pdf_rag.py formats"
        echo "Also: arka pdf ingest <path>  |  agent \"ingest document ~/path/file.docx\""
        return 1
    end
    set -l py (_arka_python)
    $py ~/.config/fish/arka_pdf_rag.py ingest $argv[1]
end
...
function pdf_ingest_dir --description "Ingest all PDFs under a directory (recursive, default ~/Documents)"
    ...
    $py ~/.config/fish/arka_pdf_rag.py batch-ingest $extra "$dir"
end
...
function codebase_ingest --description "Index a project directory for TurboQuant Q&A"
    if test (count $argv) -eq 0
        ...
        echo "Example: codebase_ingest ~/.config/fish -n fish"
        echo "Then: doc_ask --doc codebase-fish \"how does agent routing work?\""
        return 1
    end
    set -l py (_arka_python)
    $py ~/.config/fish/arka_pdf_rag.py codebase-ingest $argv
end
...
function pdf_ask --description "Ask or summarize ingested documents; optional --doc to pick one file"
    ...
    set -l py (_arka_python)
    if test -z "$doc_flag"
        set -l parsed ($py ~/.config/fish/arka_pdf_rag.py parse-ask "$question" 2>/dev/null)
    ...
    set -l py (_arka_python)
    if test -n "$doc_flag"
        set answer ($py ~/.config/fish/arka_pdf_rag.py ask $doc_flag $question | string collect)
    else
        set answer ($py ~/.config/fish/arka_pdf_rag.py ask $question | string collect)
    end
    set -l st $status
    if test $st -ne 0
...
function generate_password --description "Generate secure passwords; store/retrieve by name (encrypted vault)"
    set -l py (_arka_python)
    set -l vault ~/.config/fish/arka_password_vault.py
...
function stock_analysis --description "Stock market intelligence (stock_analysis project)"
    set -l bridge ~/.config/fish/arka_stock_bridge.py
    set -l py (_arka_python)
    if test (count $argv) -eq 0
    ...
    switch $argv[1]
        ...
        case compare
            ...
            $py ~/.config/fish/arka_predictions.py compare $amt --horizon "$hz"
        case macro events disaster
            $py ~/.config/fish/arka_macro_events.py --limit (test (count $argv) -ge 2; and echo $argv[2]; or echo 8)
        case funding fundings vc deals
            $py ~/.config/fish/arka_competition_funding.py --funding-limit (test (count $argv) -ge 2; and echo $argv[2]; or echo 8)
        case competition peers rivals
            set -l cf_args
            if test (count $argv) -ge 2
                set cf_args $argv[2..-1]
            end
            $py ~/.config/fish/arka_competition_funding.py $cf_args
        case fundamentals ratios metrics
            ...
            $py ~/.config/fish/arka_stock_fundamentals.py $argv[2..-1]
        case emotion sentiment mood crowd
            $py ~/.config/fish/arka_market_emotion.py --limit (test (count $argv) -ge 2; and echo $argv[2]; or echo 20)
        case invest
            if test (count $argv) -lt 2
...
function install_skill_deps --description "Install all system dependencies for agent skills"
    ...
    mkdir -p ~/.config/fish
    touch ~/.config/fish/.skills_setup_done
...
function _agent_is_pdf_question --description "True if user wants to query ingested documents (internal)"
    ...
    set -l py (_arka_python)
    set -l parsed ($py ~/.config/fish/arka_pdf_rag.py parse-ask "$argv[1]" 2>/dev/null)
    if test $status -eq 0 -a -n "$parsed"
        set -l parts (string split \t "$parsed")
        if test (count $parts) -ge 3 -a -n "$parts[2]" -a -n "$parts[3]"
            return 0
...
function _agent_pdf_ask_cmd --description "Build pdf_ask command with optional document (internal)"
    set -l cmd "$argv[1]"
    set -l py (_arka_python)
    set -l parsed ($py ~/.config/fish/arka_pdf_rag.py parse-ask "$cmd" 2>/dev/null)
    if test $status -eq 0 -a -n "$parsed"
        set -l parts (string split \t "$parsed")
        if test (count $parts) -ge 3 -a -n "$parts[2]" -a -n "$parts[3]"
...
function _agent_chat_intent_route --description "Map question to chat skill via arka_chat intent (internal)"
    ...
    set -l intent (python3 ~/.config/fish/arka_chat.py intent "$cmd" 2>/dev/null)
...
Rules:
- Pick commands relevant to the QUESTION (CPU age -> lscpu, /proc/cpuinfo; disk space -> df; GPU -> lspci; malware/security -> ps aux, ss -tulpn, journalctl -p err -n 30, systemctl list-units --failed, last -10, ls ~/.config/autostart, command -v clamscan; app/screen time -> python3 ~/.config/fish/arka_usage.py report today).
...
function _arka_chat_ask --description "Run arka_chat.py ask; prints answer (internal)"
    ...
    set -l py (_arka_python)
    $py ~/.config/fish/arka_chat.py $py_args 2>/dev/null
end
...
function calc --description "Symbolic math via SymPy + AI explanation"
    ...
    set -l answer (python3 ~/.config/fish/arka_chat.py ask $argv 2>/dev/null)
    if test -z "$answer"
        set -l raw (python3 ~/.config/fish/arka_chat.py calc $argv 2>/dev/null)
...
function chat_reset --description "Reset Arka chat session and location context"
    python3 ~/.config/fish/arka_chat.py session-reset
end

function set_location --description "Set or refresh location/PIN for grounded search"
    if test (count $argv) -eq 0
        python3 ~/.config/fish/arka_chat.py location --refresh
        return $status
    end
    python3 ~/.config/fish/arka_chat.py location $argv[1]
end

function nearby_places --description "Offline nearby POIs from cached city map"
    if test (count $argv) -gt 0
        python3 ~/.config/fish/arka_chat.py nearby $argv[1]
    else
        python3 ~/.config/fish/arka_chat.py nearby
    end
end
...
function map_download --description "Download offline POI map for a city (Overpass/OSM)"
    ...
    python3 ~/.config/fish/arka_chat.py map download $argv[1]
end
...
function error_helper --description "Explain a Python/shell error or traceback"
    ...
    set -l answer (python3 ~/.config/fish/arka_chat.py error-explain $argv 2>/dev/null)
    if test -z "$answer"
...
function deep_queue --description "Background deep-search queue (add/list/run/results)"
    ...
    switch $argv[1]
        case add
            ...
            python3 ~/.config/fish/arka_chat.py queue add $argv[2..-1]
        case list
            python3 ~/.config/fish/arka_chat.py queue list
        case run
            python3 ~/.config/fish/arka_chat.py queue run
        case results
            python3 ~/.config/fish/arka_chat.py queue results
        case '*'
...
function _arka_agent --description "Run arka_agent.py subcommand (internal)"
    set -l py (_arka_python)
    $py ~/.config/fish/arka_agent.py $argv
end
...
function agent_handoff --description "Phone ↔ PC shared task queue"
    ...
    switch $argv[1]
        ...
        case run
            set -l py (_arka_python)
            $py ~/.config/fish/arka_talents.py handoff-run-notify
        case notify notifications
            set -l py (_arka_python)
            if test (count $argv) -ge 2; and test "$argv[2]" = read
                $py ~/.config/fish/arka_talents.py handoff-notify-read $argv[3]
            else
                $py ~/.config/fish/arka_talents.py handoff-notify-list --unread
            end
        case clear
...
function rag_setup --description "Install Firmamento TurboQuant for unified RAG"
    set -l py (_arka_python)
    $py ~/.config/fish/arka_turboquant_install.py install
end

function rag_status --description "Check TurboQuant RAG backend status"
    set -l py (_arka_python)
    $py ~/.config/fish/arka_turboquant_install.py check
end
...
function _arka_talents --description "Run arka_talents.py (internal)"
    set -l py (_arka_python)
    $py ~/.config/fish/arka_talents.py $argv
end
...
function predictions --description "Arka talent: opportunity predictions (antiques, stocks, strategy)"
    ...
    if test "$argv[1]" = history
        $py ~/.config/fish/arka_predictions.py history
        return $status
    end
    set -l flags
    set -l args $argv
    ...
    $py ~/.config/fish/arka_predictions.py run $flags -- (string join " " $args)
end
...
function web_answer --description "Answer factual questions via web lookup + AI (auto deep search when needed)"
    ...
    set -l intent (python3 ~/.config/fish/arka_chat.py intent "$question" 2>/dev/null)
    set -l action (string split -f 1 \t "$intent")
    ...
    set -l snippet (python3 ~/.config/fish/web_answer.py snippet "$question" 2>/dev/null)
...
function hyperlocal_weather --description "Weather via Open-Meteo + IP geolocation"
    set -l question (string join " " $argv)
    set -l py (_arka_python)
    set -l wx ($py ~/.config/fish/arka_chat.py weather $argv 2>/dev/null)
    if test -z "$wx"
        weather $argv
        return $status
    end
...
function web_essay --description "Write an essay on a topic via web lookup + AI"
    ...
    set -l snippet (python3 ~/.config/fish/web_answer.py snippet "$topic" 2>/dev/null)
...
function _agent_stt_quick_map --description "Fix common STT mis-hearings before routing (internal)"
    ...
    python3 ~/.config/fish/arka_stt_map.py normalize "$text"
end
        ...
        if test (count $argv) -ge 1
            switch $argv[1]
                ...
                case speak-voice voice
                    ...
                    return $status
                case tts-setup
                    python3 ~/.config/fish/edge_speak.py setup
                    python3 ~/.config/fish/indic_tts.py setup
                    return $status
                case serve
                    _agent_remote_start
                    return $status
                ...
                case models stt
                    ...
                    $wake_py ~/.config/fish/arka_wake.py --models
                    return $status
                ...
                case voice hf-voice
                    if test (count $argv) -ge 2
                        bash ~/.config/fish/arka_voice_hf.sh $argv[2..-1]
                    else
                        bash ~/.config/fish/arka_voice_hf.sh status
                    end
                    return $status
                    ...
                    if test (count $argv) -ge 2
                        switch $argv[2]
                            ...
                            case stop
                                python3 ~/.config/fish/arka_usage.py stop
                            case gnome
                                python3 ~/.config/fish/arka_usage.py gnome
                            case '*'
                                app_usage $argv[2..-1]
                        end
                    else
                ...
                case pdf document docs
                    ...
                    if test (count $argv) -ge 2
                        switch $argv[2]
                            case formats types
                                $py ~/.config/fish/arka_pdf_rag.py formats
                            case status
                                $py ~/.config/fish/arka_pdf_rag.py status
                            case list ls
                                pdf_list
                ...
                case compute cores cpu gpu
                    set -l py (_arka_python)
                    $py ~/.config/fish/arka_compute.py
                    return $status
                case stop down
                    if test (count $argv) -gt 1
                        agent $argv
...
function agent_hear --description "Run agent on speech transcript; speaks the reply"
    ...
    set -l py (_arka_python)
    set -l enriched ($py ~/.config/fish/arka_talents.py voice-enrich "$stripped")
    ...
    set -l st $status
    printf '%s\n' "$out"
    $py ~/.config/fish/arka_talents.py voice-record --user "$stripped" --assistant "$out" 2>/dev/null
    if test $st -eq 0
        _agent_speak_reply "$out"
    else if test -n "$out"
        _agent_speak_reply "$out"
...
function _arka_wake_python --description "Python for wake listener venv (internal)"
    set -l py ~/.config/fish/venv-arka/bin/python3
    if test -x "$py"
        echo "$py"
    else
        echo python3
...
function agent --description "Run commands safely: executes safe commands automatically, prompts for dangerous ones"
    ...
    if test (count $argv) -eq 0
        ...
        if test "$call" != agent
            ...
            echo (set_color cyan)"💡 Set "(set_color --bold)"AGENT_NAME"(set_color normal)" in ~/.config/fish/.env to call this agent by a custom name (speech / multi-agent)."(set_color normal)
        end
...
if not test -f ~/.config/fish/.skills_setup_done
    install_skill_deps
end
