...
class ArkaRemoteHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path == "/v1/handoff":
            ...
            from arka_agent import handoff_api_list
        ...
        if path == "/v1/notifications":
            sys.path.insert(0, str(Path.home() / ".config" / "fish"))
            from arka_talents import handoff_notifications_list
    ...
    def do_POST(self) -> None:
        ...
        if path == "/v1/handoff":
            try:
                ...
                return
            sys.path.insert(0, str(Path.home() / ".config" / "fish"))
            from arka_agent import handoff_api_add, handoff_api_list
        ...
        if path == "/v1/notifications/read":
            try:
                ...
                data = {}
            sys.path.insert(0, str(Path.home() / ".config" / "fish"))
            from arka_talents import handoff_notifications_mark_read
        ...
        if path == "/v1/inbox":
            ...
            if not from_num or not text:
                ...
                return
            sys.path.insert(0, str(Path.home() / ".config" / "fish"))
            from arka_whatsapp_inbox import handle_inbox_message
