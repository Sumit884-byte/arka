...
description: "Install Arka, connect it to Cursor over MCP, and use 70+ local terminal skills. Start with Quickstart or the AI agent guide."
keywords: ["arka-agent", "AI terminal agent", "MCP", "Cursor", "CLI"]
icon: "book-open"
boost: 2
---

Arka is a **cross-platform AI agent** for your terminal. Say what you want in plain English — Arka routes the request to a local skill, runs it, and can speak the answer back.

Commands run on your machine. LLM calls go through a fallback list of providers. You do not need Docker for daily use.

Language models should start at [llms.txt](/llms.txt) and open one page (append `.md` for Markdown).
...
<CardGroup cols={2}>
  ...
  <Card title="AI agents / MCP" icon="robot" href="/guides/ai-agents">
    How Cursor and Claude should call Arka tools.
  </Card>
</CardGroup>
...
Use the sidebar tabs: **Get Started**, **Guides**, **Concepts**, and **Reference**. Switch language for 中文 or हिंदी. Agents should use [llms.txt](/llms.txt) instead of crawling the sidebar.
...
<Note>
  Open source under MIT. PyPI package: `arka-agent`. Documentation: [arka-agent.mintlify.site](https://arka-agent.mintlify.site).
</Note>
