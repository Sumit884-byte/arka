...
When you switch providers with `arka provider set groq` (or `ai-pref groq` in fish), Arka tries to **keep the same model** on the new provider before falling back to that provider's default. Disable with `arka provider set groq --no-keep-model` or `AI_KEEP_MODEL_ON_PROVIDER_SWITCH=0`.
...
| `arka predictions` | Stock and forecast analysis |
