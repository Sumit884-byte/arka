...
Arka's `LlmFallbackEngine` manages all LLM calls across **24 providers** with automatic failover, API key rotation, and per-task or per-skill model overrides.

## Supported providers

Anthropic, OpenAI, Gemini, Groq, xAI, DeepSeek, Moonshot, Z.AI, MiniMax, Venice, Mistral, Cohere, Together, Fireworks, Perplexity, Hugging Face, OpenRouter, Bedrock, Azure, LiteLLM, Ollama, LM Studio, vLLM, and vLLM Cloud.
