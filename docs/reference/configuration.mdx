...
| Model policy | Try local before hosted (`ARKA_MODEL_POLICY=local-first`) | `hosted-first`, `local-only`, `hosted-only` (see [Find free models](/guides/free-models)) |
...
| Web search timeout | 20 s per DuckDuckGo call (`DDGS_TIMEOUT=20`) | any integer ≥ 5 |
| Web search retries | 2 attempts before giving up (`DDGS_RETRIES=2`) | any integer ≥ 1 |

Web search failures are non-fatal: if DuckDuckGo times out or errors on every retry, chat logs the error and answers from model knowledge instead of stalling. Tune both knobs when you're on a slow or flaky network:
