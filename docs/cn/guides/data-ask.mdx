...
| 命令 | 说明 |
| ---- | ---- |
| `arka data_ask users.csv "how many rows?"` | 对文件提问 |
| `arka data_ask ./data/ --format csv "average salary?"` | 只查询文件夹中的 CSV 文件 |
| `arka data_ask ./reports/ --formats csv,json "list all columns"` | 按多种格式筛选 |
| `arka data_ask folder ./exports --question "how many records total?"` | 显式的文件夹模式 |
| `ask_data sales.json "total by category"` | `data_ask` 的别名 |
| `arka query_data report.tsv` | 未提供问题时进行摘要 |
| `arka analyze_data data.jsonl "top 5 products"` | `data_ask` 的别名 |
