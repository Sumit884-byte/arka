...
| `arka_heartbeat` | 智能体心跳 — ping 活动或读取健康状态快照 |
| `arka_sessions` | 频道会话 — 列出、状态、上下文、推送回合、重置 |
| `arka_routines` | 列出计划例程（schedule → action） |
| `arka_session_memory` | OpenClaw Markdown 内存 — 追加、搜索、上下文、状态 |
| `arka_subagent` | Hermes 子智能体委派 — 生成、列出、状态 |
