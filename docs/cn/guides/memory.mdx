...
| `arka supermemory status` | 显示后端模式（api / local） |
| `arka supermemory remember` | 显式云端记忆 |
| `arka supermemory recall` | 显式云端回忆 |
| `arka "semantic memory reindex"` | 在本地缓存上重建语义索引 |
...
| 层 | 命令 / 存储 | 何时使用 |
| ----- | --------------- | ----------- |
| 事实（长期） | `arka memory remember`，Supermemory | 偏好、数周内稳定的知识 |
| 会话笔记 | `arka session-memory` | Markdown 笔记、每日日志（`MEMORY.md`） |
| 频道对话 | `arka session` | 每个聊天/线程的连续性（webhook、CLI、Telegram 桥） |
...
| `arka memory status` | 显示所有层的计数 |
| `arka memory scratchpad list` | 列出作用域内的工作流条目 |
| `arka memory promote <id>` | 将草稿板条目提升为全局事实 |
...
现有命令（`arka session-memory`、`arka session`）仍可与统一记忆一起使用。
...
| 命令 | 示例 |
| ------- | ------- |
| `arka memory scope status` | 信任上限、草稿板数量 |
| `arka memory scratchpad list` | 列出作用域条目 |
| `arka memory scratchpad list --team research` | 按团队筛选 |
| `arka memory promote <id>` | 将草稿板条目提升为全局事实 |
