...
| `arka "SUBAGENT_MAX"` | 并发运行的 Agent 超过限制 |
| `arka "SUBAGENT_MAX_CHARS"` | 任务文本过长 |
...
| 变量 | 默认值 | 用途 |
| -------- | ------- | ------- |
| `arka "SUBAGENT_ENABLED"` | `1` | 启用（`0` 禁用） |
| `arka "SUBAGENT_DIR"` | `~/.cache/fish-agent/subagents` | 存储根目录 |
| `arka "SUBAGENT_MAX"` | `3` | 最大并发后台 Agent 数 |
| `arka "SUBAGENT_TIMEOUT"` | `600` | 每个 Agent 超时时间（秒） |
| `arka "SUBAGENT_SECURITY"` | `1` | 对生成任务应用安全门禁 |
