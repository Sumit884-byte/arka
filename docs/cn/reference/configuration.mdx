...
| 模型策略 | 优先尝试本地再托管（`ARKA_MODEL_POLICY=local-first`） | `hosted-first`、`local-only`、`hosted-only`（参见[查找免费模型](/cn/guides/free-models)） |
...
| 网络搜索超时 | 每次 DuckDuckGo 调用 20 秒（`DDGS_TIMEOUT=20`） | 任意 ≥ 5 的整数 |
| 网络搜索重试 | 放弃前尝试 2 次（`DDGS_RETRIES=2`） | 任意 ≥ 1 的整数 |

网络搜索失败不会阻塞聊天：若 DuckDuckGo 每次重试都超时或报错，聊天会记录错误并改用模型知识回答，而不是让回复卡住。在网络较慢或不稳定时可调高两项：
