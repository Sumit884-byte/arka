...
```bash
arka select_model                    # show recommendations
arka select_model --apply            # write to llm-skill-models.json
arka select_model --json             # machine-readable output
arka select_model probe              # hardware snapshot only
arka select_model --local            # choose the strongest installed local model that fits
arka select_model --local --run "summarize this repo"  # run a prompt with it via Ollama
```
