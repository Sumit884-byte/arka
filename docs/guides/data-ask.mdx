...
| Command | Description |
| --- | --- |
| `arka data_ask users.csv "how many rows?"` | Ask a question about a file |
| `arka data_ask ./data/ --format csv "average salary?"` | Query only CSV files in a folder |
| `arka data_ask ./reports/ --formats csv,json "list all columns"` | Filter by multiple formats |
| `arka data_ask folder ./exports --question "how many records total?"` | Explicit folder mode |
| `ask_data sales.json "total by category"` | Alias for `data_ask` |
| `arka query_data report.tsv` | Summarize when no question is given |
| `arka analyze_data data.jsonl "top 5 products"` | Alias for `data_ask` |
