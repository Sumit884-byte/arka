...
description: "Terminal coding workflow with Arka — arka code init, agent mode, repo health checks, CI fixes, and Cursor MCP integration for a full edit-review-test loop without leaving the CLI."
...
<Steps>
  ...
  <Step title="Initialize a code project">
    ...
    Or use the legacy alias: `arka code init .`. The goal agent, `self improve --apply`, and other write skills stay inside this folder until you `arka code clear` or init another path.
  </Step>
...
| Focused TUI workspace | `arka dev tui .` (alias: `arka coding-tui .`) |
...
`arka dev ship` runs staged review plus changed-only CI in one step. `arka dev review` (or `arka review --staged`) checks the diff against project rules and surfaces **test-gap** hints for changed source files without matching tests. [Council](/guides/council) runs multi-persona deliberation for design decisions. [Teammate review](/guides/teammate-review) flags cross-service impact using workspace discovery.

See the full [Developer loop](/guides/dev) guide.
...
`arka dev test` runs changed-only CI gates (same as `arka ci --changed`). Use `repo_health` for generic lint/test detection in any project. `arka dev pr babysit` loops on GitHub CI until the PR is merge-ready. See [Repo health](/guides/repo-health) and [Developer loop](/guides/dev).

<Note>
  Install pre-commit hooks for incremental gates: `arka dev hooks install` runs `arka ci --changed` and `arka review --staged --fail-on-hints` on each commit.
</Note>
