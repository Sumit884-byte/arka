...
This page covers dispatch-backed skills and opt-in personal tools. Routing table and session flow: [AI agent guide](/guides/ai-agents).
...
2. Inspect that result. Confirm success or failure from the output.
...
| Situation | Tool |
| --- | --- |
| Unsure which Arka tool fits | `arka_route` with full user `prompt` |
| Know the skill name | `arka_skill` with `skill` + optional `args` |
| Need live tool list + rules | `arka_capabilities` |
| Repo layout / symbols | `arka_repo_map` |
| Fuzzy-find a project folder | `arka_tech_stack` (`action: search`) |
| OCR or document RAG | `arka_ocr`, `arka_rag` (local paths) |
| Patch files | `arka_edit_guard` → `arka_apply_patch` |

Full decision table and examples: [AI agent guide](/guides/ai-agents#routing-decision-table).
