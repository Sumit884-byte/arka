...
Heartbeat data is written to `~/.cache/fish-agent/heartbeat.json`. Subsystems like session memory and routines call `ping()` automatically on activity.
...
### `metadata.openclaw` (or `metadata.arka`)

The same fields can live under `metadata.openclaw` for compatibility with OpenClaw-style manifests:
...
Arka merges `metadata.openclaw` into the top-level `requires`, `os`, and `permissions` fields at parse time.
