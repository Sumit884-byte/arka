...
Export traces, metrics, and logs to [SigNoz](https://signoz.io). **On by default** to local OTLP at `http://127.0.0.1:4318` when SigNoz is running.
...
```env
# Enabled by default — override only when needed
SIGNOZ_ENDPOINT=http://localhost:4318
SIGNOZ_UI_URL=http://localhost:8080
OTEL_SERVICE_NAME=arka
# OTEL_SDK_DISABLED=true          # disable all OTLP export
# OTEL_TRACES_ENABLED=0           # disable traces (metrics/logs follow)
...
Every dispatched skill records a bounded skill name, duration, exit status, execution kind
