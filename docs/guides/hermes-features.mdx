...
| `arka "SUBAGENT_MAX"` | Concurrent running agents exceed limit |
| `arka "SUBAGENT_MAX_CHARS"` | Task text too long |
...
| Variable | Default | Purpose |
| --- | --- | --- |
| `arka "SUBAGENT_ENABLED"` | `1` | Enable (`0` to disable) |
| `arka "SUBAGENT_DIR"` | `~/.cache/fish-agent/subagents` | Storage root |
| `arka "SUBAGENT_MAX"` | `3` | Max concurrent background agents |
| `arka "SUBAGENT_TIMEOUT"` | `600` | Per-agent timeout (seconds) |
| `arka "SUBAGENT_SECURITY"` | `1` | Apply security gates to spawn tasks |
