...
|---------|------|--------|---------|
| `/test [path]` | strict read-only | No — direct execution | Never |
| `/run tests` | flexible read-only | Yes — readonly goal agent | Never |
| `/run tests --fix` | fix mode | Yes | Allowed |
| plain `test` or `tests` | same as `/test` | No | Never |
| plain `run tests` | same as `/run tests` | Yes (readonly) | Never |
...
e.g. `/test tests/test_foo.py`. Output ends with
`Test run complete (read-only, no files modified)`.

`/run tests` lets the readonly goal agent pick a strategy — full pytest, a
focused module, `arka ci --changed`, or a repo_health-detected runner. Add
`--fix` to allow the agent to edit files and repair failures.
