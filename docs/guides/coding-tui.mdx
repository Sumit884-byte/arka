...
|---------|------|--------|---------|
| `/test [path]` | strict read-only, then auto-fix on failure | Fix pass only | Fix pass only |
| `/test --no-fix [path]` | strict read-only only | No | Never |
| `/test --max-fixes N` | up to N fix passes (default 1) | Fix passes only | Fix passes only |
| `/run tests` | flexible read-only, then auto-fix on failure | Yes | Fix pass only |
| `/run tests --no-fix` | flexible read-only only | Yes (readonly) | Never |
| `/run tests --fix` | fix mode from the start | Yes | Allowed |
| `/run tests --max-fixes N` | up to N fix passes after readonly run | Yes | Fix passes only |
...
| `/ship` | staged review + changed CI | No | Never |
| `/ship --fix` | ship with goal-agent CI fix | Yes on CI failure | Allowed on fix |
| `/ci` | run changed-only CI gates | No | Never |
| `/ci preview` | preview gate list only | No | Never |
| `/review --fail` | exit TUI if review hints present | No | Never |
...
i/N** — up to `--max-fixes` writable goal-agent passes (default **1**) — then
re-runs tests read-only to show the result. Use `/test --no-fix` to skip the
...
If tests still fail after all fix passes:
...
project is initialized. `/ci` runs changed-only gates; `/ci preview` lists gates
without executing. `/review` summarizes staged changes with test-gap hints.
`/ship` runs review + CI before commit. `/history` lists commands from the
current session; `/clear` clears the screen and resets session state.

The coding agent prints a completion/failure summary after execution and suggests
`arka dev test` for incremental verification.
