...
plan when prompted, then use `/run` to execute. `/plan` calls the LLM in
plan-only mode and prints a structured plan (summary, files, steps) without
making changes. When the LLM is unavailable, Arka falls back to a local
...
The coding agent prints a completion/failure summary after `/run` and suggests
