...
| `arka_heartbeat` | Agent heartbeat — ping activity or read health snapshot |
| `arka_sessions` | Channel sessions — list, status, context, push turns, reset |
| `arka_routines` | List scheduled routines (schedule → action) |
| `arka_session_memory` | OpenClaw markdown memory — append, search, context, status |
| `arka_subagent` | Hermes sub-agent delegation — spawn, list, status |
