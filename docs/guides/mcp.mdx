...
Arka can connect to **any MCP server** from the terminal. This is the same [Model Context Protocol](https://modelcontextprotocol.io) used by Cursor, Claude Desktop, and other AI clients.

Arka is both an MCP client (call SigNoz, filesystem, and other servers) and an MCP server (expose skills to Cursor). Config is `~/.config/arka/mcp.json`. Start with `arka mcp list`, `arka mcp add`, `arka mcp tools`, `arka mcp call`. Tool routing: [AI agent guide](/guides/ai-agents). IDE workflow: [How to code with Arka](/guides/code-with-arka).
...
Arka reuses the existing HTTP MCP client for remote servers and a built-in newline-delimited JSON-RPC client for stdio. No extra dependencies are required.
...
Other agents (Cursor, Claude Desktop, OpenClaw, Codex, etc.) can call Arka skills and memory over MCP. This uses the same stdio transport Arka uses as an MCP **client**.
...
| `arka_music_generate` | Generate music — Pollinations elevenmusic or ffmpeg tone synthesis |
| `arka_meme` | Local meme templates — Drake, comparison, vibe-coding, stock photos optional |
| `arka_infographic` | Adaptive infographic PNG — grid/radial layout from item count |
| `arka_media_styles` | List meme, video, and infographic visual style presets |
| `arka_tech_stack` | Fuzzy-find project folder, confirm if not exact, read manifests, suggest stack |
| `arka_fetch_lyrics` | Fetch song lyrics, translate them, optionally generate a translated cover |
| `arka_safety_advice` | Curated crisis guidance — domestic violence, harassment (fixed playbook, not LLM legal advice) |
| `arka_local_music` | Offline music — local ffmpeg melodies (no cloud API) |
