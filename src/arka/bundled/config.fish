...
function _arka_speak_lang --description "Set or list Arka voice language (internal)"
    ...
    set -l code (python3 $_ARKA_ROOT/indic_tts.py resolve-lang $argv[1] 2>/dev/null)
    if test -z "$code"
...
function _arka_speak_voice --description "Set or list Arka neural voice (internal)"
    if test (count $argv) -eq 0
        python3 $_ARKA_ROOT/edge_speak.py voices
        return 0
    end
    set -l voice $argv[1]
...
function _arka_start_all --description "Start remote server + wake listener (internal)"
    ...
    if test $quiet -eq 0
        ...
        bash $_ARKA_ROOT/arka_boot.sh start
        sleep 1
        _arka_status_all
    else
        env ARKA_START_QUIET=1 bash $_ARKA_ROOT/arka_boot.sh start 2>/dev/null
    end
end

function _arka_stop_all --description "Stop all Arka background services (internal)"
    bash $_ARKA_ROOT/arka_boot.sh stop
    _agent_usage_stop
...
" 2>/dev/null)
    ...
    echo "#   mkdir -p ~/arka && scp YOU@PC_IP:$_ARKA_ROOT/arka_phone.py ~/arka/"
...
function _agent_remote_start --description "Start Arka remote server for phone STT/TTS (internal)"
    set -l script $_ARKA_ROOT/arka_remote_server.py
...
function _agent_remote_stop --description "Stop Arka remote server (internal)"
    set -l pidfile ~/.cache/fish-agent/arka_remote.pid
    python3 $_ARKA_ROOT/arka_remote_server.py stop 2>/dev/null
    if test -f "$pidfile"
        set -l pid (cat "$pidfile" 2>/dev/null)
        kill "$pid" 2>/dev/null
        rm -f "$pidfile"
...
function _agent_llm_route --description "Interpret NL command to skill/shell via Agno (internal)"
    ...
    set -l out ($py $_ARKA_ROOT/arka_llm.py route "$cmd" --skills "$available_skills" 2>/dev/null)
    if test -n "$out"
        _agent_clean_llm_output "$out"
        return 0
    end
...
function weather --description "Get current weather or N-day forecast (Open-Meteo)"
    set -l py (_arka_python)
    if test (count $argv) -eq 0
        $py $_ARKA_ROOT/arka_chat.py weather
        return $status
    end
    $py $_ARKA_ROOT/arka_chat.py weather $argv
end
...
function internet_enhance --description "Artificial Internet Enhancements (AIE): automate desktop/internet helpers"
    set -l py (_arka_python)
    if test (count $argv) -eq 0
        $py $_ARKA_ROOT/arka_aie.py status
        return $status
    end
    switch $argv[1]
        case help -h --help
...
function youtube_bulk --description "Bulk download YouTube playlists/channels (yt-dlp + web UI)"
    set -l py (_arka_python)
    if test (count $argv) -eq 0
        $py $_ARKA_ROOT/arka_youtube_bulk.py status
        return $status
    end
    switch $argv[1]
        case help -h --help
        ...
        case status start stop open download library logs
            $py $_ARKA_ROOT/arka_youtube_bulk.py $argv
            return $status
        case '*'
            echo (set_color red)"Unknown subcommand: $argv[1]"(set_color normal)
            youtube_bulk help
...
function youtube_download --description "Download a single YouTube video or shorts link (yt-dlp)"
    ...
    set -l py (_arka_python)
    $py $_ARKA_ROOT/arka_youtube.py download $argv
end
...
function _arka_summarize_media --description "Transcribe + summarize a local media file (internal)"
    ...
    $py $_ARKA_ROOT/arka_media.py summarize "$file" -q "$q" --save $yt_args
end
...
function generate_image --description "Generate images via Google Nano Banana (Gemini API) or Pollinations"
    ...
    set -l py (_arka_python)
    $py $_ARKA_ROOT/arka_generate_image.py $flags -- "$prompt"
end
...
function generate_video --description "Generate real AI video (Pollinations or Gemini Veo — no fake slideshows)"
    ...
    set -l prompt (string join ' ' -- $prompt_parts)
    set -l py (_arka_python)
    $py $_ARKA_ROOT/arka_generate_video.py $flags -- "$prompt"
end
...
function youtube_transcript --description "Fetch or summarize a YouTube video transcript"
    ...
    set -l py (_arka_python)
    $py $_ARKA_ROOT/arka_youtube.py $argv
end
...
function media_transcript --description "Transcribe or summarize local mp3/mp4/audio/video"
    ...
    if test "$argv[1]" = --setup-local
        set -l py (_arka_python)
        $py $_ARKA_ROOT/arka_media.py setup-local
        return $status
    end
    set -l py (_arka_python)
    ...
    if test "$action" = summarize
        $py $_ARKA_ROOT/arka_media.py summarize $args
    else
        $py $_ARKA_ROOT/arka_media.py transcript $args
    end
end
...
function folder_summarize --description "Summarize all media files in a folder into one digest"
    ...
    set -l py (_arka_python)
    $py $_ARKA_ROOT/arka_batch_summarize.py folder $argv
end
...
function summarize_url --description "Summarize a web page or article URL"
    ...
    set -l py (_arka_python)
    $py $_ARKA_ROOT/arka_summarize.py $argv
end
...
function pdf_list --description "List ingested documents (filename + artifact id)"
    set -l py (_arka_python)
    $py $_ARKA_ROOT/arka_pdf_rag.py list
end
...
function pdf_ingest --description "Ingest a document; auto-starts PrivateGPT and Qdrant if needed"
    if test (count $argv) -eq 0
        ...
        echo "Formats: arka pdf formats  |  python3 $_ARKA_ROOT/arka_pdf_rag.py formats"
        echo "Also: arka pdf ingest <path>  |  agent \"ingest document ~/path/file.docx\""
        return 1
    end
    set -l py (_arka_python)
    $py $_ARKA_ROOT/arka_pdf_rag.py ingest $argv[1]
end
...
function pdf_ingest_dir --description "Ingest all PDFs under a directory (recursive, default ~/Documents)"
    ...
    $py $_ARKA_ROOT/arka_pdf_rag.py batch-ingest $extra "$dir"
end
...
function codebase_ingest --description "Index a project directory for TurboQuant Q&A"
    ...
    set -l py (_arka_python)
    $py $_ARKA_ROOT/arka_pdf_rag.py codebase-ingest $argv
end
...
function pdf_ask --description "Ask or summarize ingested documents; optional --doc to pick one file"
    ...
    set -l py (_arka_python)
    if test -z "$doc_flag"
        set -l parsed ($py $_ARKA_ROOT/arka_pdf_rag.py parse-ask "$question" 2>/dev/null)
    ...
    set -l py (_arka_python)
    if test -n "$doc_flag"
        set answer ($py $_ARKA_ROOT/arka_pdf_rag.py ask $doc_flag $question | string collect)
    else
        set answer ($py $_ARKA_ROOT/arka_pdf_rag.py ask $question | string collect)
    end
    set -l st $status
    if test $st -ne 0
...
function stock_analysis --description "Stock market intelligence (stock_analysis project)"
    ...
    switch $argv[1]
        ...
        case compare
            ...
            $py $_ARKA_ROOT/arka_predictions.py compare $amt --horizon "$hz"
        case macro events disaster
            $py $_ARKA_ROOT/arka_macro_events.py --limit (test (count $argv) -ge 2; and echo $argv[2]; or echo 8)
        case funding fundings vc deals
            $py $_ARKA_ROOT/arka_competition_funding.py --funding-limit (test (count $argv) -ge 2; and echo $argv[2]; or echo 8)
        case competition peers rivals
            set -l cf_args
            if test (count $argv) -ge 2
                set cf_args $argv[2..-1]
            end
            $py $_ARKA_ROOT/arka_competition_funding.py $cf_args
        case fundamentals ratios metrics
            ...
            $py $_ARKA_ROOT/arka_stock_fundamentals.py $argv[2..-1]
        case emotion sentiment mood crowd
            $py $_ARKA_ROOT/arka_market_emotion.py --limit (test (count $argv) -ge 2; and echo $argv[2]; or echo 20)
        case invest
            if test (count $argv) -lt 2
...
function _agent_pdf_ask_cmd --description "Build pdf_ask command with optional document (internal)"
    set -l cmd "$argv[1]"
    set -l py (_arka_python)
    set -l parsed ($py $_ARKA_ROOT/arka_pdf_rag.py parse-ask "$cmd" 2>/dev/null)
    if test $status -eq 0 -a -n "$parsed"
        set -l parts (string split \t "$parsed")
        if test (count $parts) -ge 3 -a -n "$parts[2]" -a -n "$parts[3]"
...
function _agent_chat_intent_route --description "Map question to chat skill via arka_chat intent (internal)"
    ...
    set -l intent (python3 $_ARKA_ROOT/arka_chat.py intent "$cmd" 2>/dev/null)
...
function calc --description "Symbolic math via SymPy + AI explanation"
    ...
    set -l answer (python3 $_ARKA_ROOT/arka_chat.py ask $argv 2>/dev/null)
    if test -z "$answer"
        set -l raw (python3 $_ARKA_ROOT/arka_chat.py calc $argv 2>/dev/null)
...
function chat_reset --description "Reset Arka chat session and location context"
    python3 $_ARKA_ROOT/arka_chat.py session-reset
end

function set_location --description "Set or refresh location/PIN for grounded search"
    if test (count $argv) -eq 0
        python3 $_ARKA_ROOT/arka_chat.py location --refresh
        return $status
    end
    python3 $_ARKA_ROOT/arka_chat.py location $argv[1]
end

function nearby_places --description "Offline nearby POIs from cached city map"
    if test (count $argv) -gt 0
        python3 $_ARKA_ROOT/arka_chat.py nearby $argv[1]
    else
        python3 $_ARKA_ROOT/arka_chat.py nearby
    end
end
...
function map_download --description "Download offline POI map for a city (Overpass/OSM)"
    ...
    python3 $_ARKA_ROOT/arka_chat.py map download $argv[1]
end
...
function error_helper --description "Explain a Python/shell error or traceback"
    ...
    set -l answer (python3 $_ARKA_ROOT/arka_chat.py error-explain $argv 2>/dev/null)
    if test -z "$answer"
...
function deep_queue --description "Background deep-search queue (add/list/run/results)"
    ...
    switch $argv[1]
        case add
            ...
            python3 $_ARKA_ROOT/arka_chat.py queue add $argv[2..-1]
        case list
            python3 $_ARKA_ROOT/arka_chat.py queue list
        case run
            python3 $_ARKA_ROOT/arka_chat.py queue run
        case results
            python3 $_ARKA_ROOT/arka_chat.py queue results
        case '*'
...
function agent_handoff --description "Phone ↔ PC shared task queue"
    ...
    switch $argv[1]
        ...
        case run
            set -l py (_arka_python)
            $py $_ARKA_ROOT/arka_talents.py handoff-run-notify
        case notify notifications
            set -l py (_arka_python)
            if test (count $argv) -ge 2; and test "$argv[2]" = read
                $py $_ARKA_ROOT/arka_talents.py handoff-notify-read $argv[3]
            else
                $py $_ARKA_ROOT/arka_talents.py handoff-notify-list --unread
            end
        case clear
...
function rag_setup --description "Install Firmamento TurboQuant for unified RAG"
    set -l py (_arka_python)
    $py $_ARKA_ROOT/arka_turboquant_install.py install
end

function rag_status --description "Check TurboQuant RAG backend status"
    set -l py (_arka_python)
    $py $_ARKA_ROOT/arka_turboquant_install.py check
end
...
function _arka_talents --description "Run arka_talents.py (internal)"
    set -l py (_arka_python)
    $py $_ARKA_ROOT/arka_talents.py $argv
end
...
function predictions --description "Arka talent: opportunity predictions (antiques, stocks, strategy)"
    ...
    if test "$argv[1]" = history
        $py $_ARKA_ROOT/arka_predictions.py history
        return $status
    end
    set -l flags
    set -l args $argv
    ...
    $py $_ARKA_ROOT/arka_predictions.py run $flags -- (string join " " $args)
end
...
function web_answer --description "Answer factual questions via web lookup + AI (auto deep search when needed)"
    ...
    set -l snippet (python3 $_ARKA_ROOT/web_answer.py snippet "$question" 2>/dev/null)
...
function hyperlocal_weather --description "Weather via Open-Meteo + IP geolocation"
    set -l question (string join " " $argv)
    set -l py (_arka_python)
    set -l wx ($py $_ARKA_ROOT/arka_chat.py weather $argv 2>/dev/null)
    if test -z "$wx"
        weather $argv
        return $status
    end
...
function web_essay --description "Write an essay on a topic via web lookup + AI"
    ...
    set -l snippet (python3 $_ARKA_ROOT/web_answer.py snippet "$topic" 2>/dev/null)
...
function _agent_stt_quick_map --description "Fix common STT mis-hearings before routing (internal)"
    ...
    python3 $_ARKA_ROOT/arka_stt_map.py normalize "$text"
end
        ...
        if test (count $argv) -ge 1
            switch $argv[1]
                ...
                case speak-voice voice
                    ...
                    return $status
                case tts-setup
                    python3 $_ARKA_ROOT/edge_speak.py setup
                    python3 $_ARKA_ROOT/indic_tts.py setup
                    return $status
                case serve
                    _agent_remote_start
                    return $status
                ...
                case models stt
                    set -l wake_py (_arka_wake_python)
                    $wake_py $_ARKA_ROOT/arka_wake.py --models
                    return $status
                ...
                case pdf document docs
                    ...
                    if test (count $argv) -ge 2
                        switch $argv[2]
                            case formats types
                                $py $_ARKA_ROOT/arka_pdf_rag.py formats
                            case status
                                $py $_ARKA_ROOT/arka_pdf_rag.py status
                            case list ls
                                pdf_list
...
function _agent_listen_start --description "Start continuous wake-word listener (internal)"
    set -l call (_agent_call_name)
    set -l script $_ARKA_ROOT/arka_wake.py
