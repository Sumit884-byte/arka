...
def _serve_env() -> dict[str, str]:
    ...
    settings_folders = [str(override_dir), str(privategpt_home())]
...
def _turboquant_rag():
    from arka_turboquant_rag import use_turboquant
...
def _extract_document_text(path: Path) -> str | None:
    ...
    if kind == "native":
        ext = path.suffix.lower()
        if ext == ".pdf":
            from arka_turboquant_rag import extract_pdf_text

            return extract_pdf_text(path)
        if ext == ".docx":
            from arka_turboquant_rag import extract_docx_text
...
def _index_document_turboquant(path: Path, text: str | None = None) -> tuple[bool, str]:
    from arka_turboquant_rag import index_document_text, sanitize_artifact
...
def _resolve_turboquant_document(ref: str | None) -> tuple[str | None, str | None, str | None]:
    from arka_turboquant_rag import resolve_indexed_document
...
def semantic_search_context(question: str, artifact: str | None = None) -> tuple[int, str]:
    if _turboquant_rag():
        from arka_turboquant_rag import search_documents
...
def cmd_codebase_ingest(path: str, name: str | None = None) -> int:
    from arka_turboquant_rag import index_codebase, use_turboquant
    ...
    from arka_turboquant_rag import sanitize_artifact
