...
FISH_DIR="${ARKA_HOME:-$SCRIPT_DIR}"
ENV_FILE="${ARKA_CONFIG_DIR:-$HOME/.config/arka}/.env"
CACHE_DIR="${ARKA_CACHE_DIR:-$HOME/.cache/fish-agent}"
mkdir -p "$CACHE_DIR"

if [[ ! -f "$ENV_FILE" && -f "$HOME/.config/fish/.env" ]]; then
    ENV_FILE="$HOME/.config/fish/.env"
fi
if [[ ! -f "$ENV_FILE" && -f "$FISH_DIR/.env.example" ]]; then
    ENV_FILE="$FISH_DIR/.env.example"
fi
