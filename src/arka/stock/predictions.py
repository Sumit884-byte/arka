...
def _stock_project_context(tickers: list[str], *, deep: bool) -> str:
    ...
    try:
        ...
        from arka.stock.bridge import gather_context
...
def gather_context(
    ...
    *,
    ...
    if domain in {"stocks", "all"} or invest:
        ...
        try:
            from arka.stock.macro_events import format_macro_event_report, is_macro_relevant_query
        ...
        try:
            from arka.stock.competition_funding import (
        ...
        try:
            from arka.stock.fundamentals import format_fundamentals_report, is_fundamentals_query
        ...
        try:
            from arka.stock.emotion import format_emotion_report, is_emotion_query
    ...
    mem = ""
    try:
        from arka.agent.talents import memory_semantic_context
...
def main() -> int:
    ...
    if args.cmd == "compare":
        ...
        try:
            from arka.stock.macro_events import format_macro_event_report
            print(format_macro_event_report(news_limit=8)[0])
        except Exception as exc:
            print(f"(Macro scan unavailable: {exc})")
        print()
        try:
            from arka.stock.competition_funding import format_competition_funding_report
            print(format_competition_funding_report([], "", funding_limit=8)[0])
        except Exception as exc:
            print(f"(Competition/funding scan unavailable: {exc})")
        print()
        try:
            from arka.stock.emotion import format_emotion_report
