...
def ci_gates(*, full: bool = False) -> list[Gate]:
    gates = [
    ...
    if full:
        gates.append(Gate("pytest-full", ["pytest", "-q", "--tb=short"]))
    return gates


def run_ci(root: Path, *, full: bool = False) -> dict:
    results: list[dict] = []
    for gate in ci_gates(full=full):
...
def ci_text(root: Path, *, full: bool = False) -> str:
    payload = run_ci(root, full=full)
...
def cmd_ci(args: argparse.Namespace) -> int:
    ...
    payload = run_ci(root, full=args.full)
    print(ci_text(root, full=args.full))
...
def cmd_review(args: argparse.Namespace) -> int:
    root = _repo_root(args.path)
    print(review_text(root, base=args.base or None, staged=args.staged))
    return 0
