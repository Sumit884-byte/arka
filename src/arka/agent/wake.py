...
def _mic_stream_sounddevice():
    ...
    from arka.integrations.mac_mic import mic_stream as _mac_stream
...
def classify_transcript(text: str) -> tuple[str, str]:
    try:
        ...
        from arka.agent.stt_map import classify_phrase
...
def contains_wake(text: str) -> bool:
    try:
        sys.path.insert(0, str(Path(__file__).resolve().parent))
        from arka.agent.stt_map import normalize_stt, strip_wake
...
def sarvam_transcribe(pcm: bytes) -> str:
    ...
    try:
        ...
        sys.path.insert(0, str(Path(__file__).resolve().parent))
        from arka.voice.sarvam_stt import transcribe_pcm
...
def assemblyai_transcribe(pcm: bytes) -> str:
    if len(pcm) < SAMPLE_RATE // 2:
        return ""
    try:
        from arka.agent.assemblyai_stt import transcribe_pcm
...
def _start_aai_command_session():
    if not _use_assemblyai_commands():
        return None
    try:
        from arka.agent.assemblyai_stt import RealtimeCommandSession
...
def normalize_stt_transcript(text: str) -> str:
    try:
        sys.path.insert(0, str(Path(__file__).resolve().parent))
        from arka.agent.stt_map import normalize_stt
...
def listen_loop_streaming() -> bool:
    ...
    try:
        from arka.agent.assemblyai_stt import (
            _keyterms,
...
def main() -> int:
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--list-mics":
        from arka.integrations.mac_mic import list_input_devices, permission_hint
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--mic-test":
        reexec_in_venv()
        from arka.integrations.mac_mic import mic_selftest
