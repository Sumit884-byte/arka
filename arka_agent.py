...
def research(question: str, *, doc: str | None = None, path: str | None = None, deep: bool = False) -> None:
    ...
    if media_path and Path(media_path).expanduser().is_file():
        try:
            from arka_media import _load_cached_transcript, transcribe_file
            from arka_media_qa import retrieve_transcript_context
...
def code_agent(goal: str, *, repo: str | None = None, ingest: bool = False) -> int:
    ...
    if ingest:
        proc = subprocess.run(
            [_py(), str(FISH_DIR / "arka_pdf_rag.py"), "codebase-ingest", str(cwd), "-n", doc_name],
            timeout=600,
        )
