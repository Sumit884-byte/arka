...
class DevToolsTests(unittest.TestCase):
    def test_route_command(self) -> None:
        ...
        self.assertEqual(dt.route_command("run ci"), "ci")
        ...
        self.assertEqual(dt.route_command("review my PR"), "review")
        self.assertEqual(dt.route_command("PR review"), "review")
        self.assertEqual(dt.route_command("route audit"), "route_audit")
        self.assertEqual(dt.route_command("audit routing coverage"), "route_audit")
        self.assertEqual(dt.route_command("routing audit"), "route_audit")
        ...
        self.assertEqual(dt.route_command("arka api tools"), "dev_tools list")
        self.assertEqual(dt.route_command("observability tools"), "dev_tools list")
        self.assertEqual(dt.route_command("dev tools list"), "dev_tools list")
        self.assertEqual(dt.route_command("show developer tools"), "dev_tools list")
        ...
        self.assertFalse(dt.wants_dev_tools("explain ci failure"))
        self.assertFalse(dt.wants_dev_tools("fix ci until green"))
    ...
    def test_router_symbolic(self) -> None:
        with mock.patch.dict("os.environ", {"ROUTE_MODE": "symbolic_only"}, clear=False):
            ...
            self.assertEqual(route("run ci").skill.split()[0], "ci")
            self.assertEqual(route("review staged").skill.split()[0], "review")
            self.assertEqual(route("review my PR").skill.split()[0], "review")
            self.assertEqual(route("PR review").skill.split()[0], "review")
            self.assertEqual(route("route audit").skill.split()[0], "route_audit")
            self.assertEqual(route("audit routing").skill.split()[0], "route_audit")
            self.assertEqual(route("check developer setup").skill.split()[0], "dev_doctor")
            self.assertEqual(route("show Postman and Insomnia API testing tools").skill, "dev_tools list")
            self.assertEqual(route("arka api tools").skill, "dev_tools list")
            self.assertEqual(route("observability tools").skill, "dev_tools list")
